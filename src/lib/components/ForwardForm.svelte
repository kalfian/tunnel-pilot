<script lang="ts">
  ...
  import { forwards } from "../stores/forwards";
  ...
  let groupId = $state<string | null>(c?.groupId ?? null);
  let tags = $state<string[]>(c?.tags ? [...c.tags] : []);
  let tagDraft = $state("");
  ...
  // Existing tags across all tunnels, for type-to-create autocomplete.
  const tagSuggestions = $derived(
    [...new Set($forwards.flatMap((f) => f.tags))]
...
>
  ...
  >
    ...
    <div class="panel" hidden={tab !== "general"}>
      ...
      <p class="overline group-head">Group &amp; tags</p>
      <div class="row">
        <label class="lbl" for="f-group">Group</label>
        <div class="group-select">
          <Select
            ...
            onchange={(v) => (groupId = v === "" ? null : v)}
          />
        </div>
      </div>
      <div class="row">
        <label class="lbl" for="f-tags">Tags</label>
