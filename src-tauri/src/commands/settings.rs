...
pub async fn update_settings(
    ...
    // Dock/taskbar visibility depends on both `showInDock` and whether the
    // window is currently shown; `refresh` reads `showInDock` from AppState.
    let window_shown = app
        .get_webview_window(crate::window::MAIN_WINDOW)
        .and_then(|w| w.is_visible().ok())
        .unwrap_or(false);
    crate::platform::dock::refresh(&app, window_shown);
